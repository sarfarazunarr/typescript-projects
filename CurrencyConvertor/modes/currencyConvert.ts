...
async function convert() {
    ...
    try {
        ...
        let conversionData: { status: number, data: { conversion_rate: number, conversion_result: number } } = await axios.get(`https://v6.exchangerate-api.com/v6/[YOUR_API_KEY]/pair/${data.from}/${data.to}/${data.amount}`);
