...
async function quiz(title: string, questiondata: { id: number, question: string, choices: string[], correctAnswer: string }[]) {
    let score: number = 0;
    let result: { Qno: number, correctAnswer: string, yourAnswer: string, correct: boolean }[] = [];
        ...
        let { answer } = await inquirer.prompt([
            {
                name: "answer",
                type: "list",
                choices: data.choices.map((mcq: string) => {
                    return mcq;
                }),
